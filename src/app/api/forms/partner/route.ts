...
import { partnerSchema } from "@/lib/forms/validators";
...
export async function POST(req: Request) {
  ...
  const to = process.env.NEXT_PUBLIC_SITE_EMAIL_PARTNERSHIPS || "partnerships@lmghi.org";

  await resend.emails.send({
    from: "LMGHI Website <no-reply@lmghi.org>",
    to,
    replyTo: parsed.data.email,
    subject: `Partnership inquiry: ${parsed.data.organization}`,
